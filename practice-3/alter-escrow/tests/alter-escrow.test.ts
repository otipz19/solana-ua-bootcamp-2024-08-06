...
import { AlterEscrow } from "../target/types/alter_escrow";
import {
  Connection,
  Keypair,
  LAMPORTS_PER_SOL,
...
describe("escrow", () => {
  ...
  const program = anchor.workspace.AlterEscrow as Program<AlterEscrow>;
  ...
  const makeOfferTx = async (
    ...
    wantedAmount: BN
  ): Promise<{ offerAddress: PublicKey }> => {
  ...
  test("Offer created by Alice, delegated approve granted, no tokens taken", async () => {
  ...
  test("Bob can't take offer: Alice has not enough tokens", async () => {
    const getTokenBalance = getTokenBalanceOn(connection);
    ...
    console.log(`Eve public key: ${eve.publicKey.toBase58()}`);
    const eveUsdcAccount = getAssociatedTokenAddressSync(
      usdcMint.publicKey,
      eve.publicKey,
      false,
      TOKEN_PROGRAM
    );
    console.log(`Eve USDC wallet: ${eveUsdcAccount.toBase58()}`);
    ...
    console.log("Created Eve USDC wallet");

    await transferChecked(connection, alice, aliceUsdcAccount, usdcMint.publicKey, eveUsdcAccount, alice, 90_000_000, 6);
    console.log("Transfered USDC from Alice to Eve");

    expect(await getTokenBalance(aliceUsdcAccount)).toEqual(new BN(0));
